...
# Reserve IP address for MetalLB
...
7.  Putty into the CVM.  Eg 10.42.169.29.  Login as nutanix.  Retrieve the password from the trainer

8.	Replace the network and IP list.  Run this command to reserve the IP

    ```bash
    acli net.add_to_ip_blacklist Primary_xxx ip_list=x.x.x.x,x.x.x.x

9.  This is the picture running the command
